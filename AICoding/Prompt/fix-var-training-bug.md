# round 1
背景：我刚刚在服务端按照var\README.md (155-165) 训练了var，但是效果很不好，训练日志如下：
var\local_output\stdout.txt (4360-4368)，我之所以说训练效果不好，是因为在对比重建图中（也就是var\local_output\cond_and_scale[0]_dont_depend_on_LR_VAE 文件夹里的对比图（从服务断拷贝到我现在本地）完全看不出有任何训练成功的样子，不是模糊，不是不精确，是根本就是一团无意义的噪声黑白（重建图中间那一列）。
ep0000_it016799_comparison.png和ep0000_it000000_comparison.png几乎没什么区别

请帮我看看问题出在哪里，如果现在的信息不够，可以多加一些调试信息，然后我帮你运行。



注意（额外信息）：

你刚刚给我的代码commit为6a102041624f5c68ef1e96bc56516bba900f634a，在此基础上，我又有新的几个提交（主要是代码运行不起来有bug）。
分别为：
c15a13747f40f8caa88dc4e21a075be056f756a8
76fac2ebb8a4045dcc4a6caf0602010b5e7f269b
46074b4885aa266477c0e08902555e40c45bcd24 fix first scale shape issue
2e61b5e9282af9b8455d8b0a178e6a54f0481d2c basic.py:fix KeyError: '((1, 4, 4), (1, 5, 5), ...)', and other small fix

其中46074b这次提交我觉得有大问题，它直接将sos 一模一样复制了16个为了匹配输入，我觉得这明显是不对的，但是我暂时没有想到应该怎么去改进它，请你给我方案（先不要自己主动修改，可以先给我几个方案让我选，总之我觉得现在这样肯定是不行的），我不知道它会不会是导致我这次训练有问题的唯一凶手，但起码是凶手之一。
2e61b5这次提交我不知道有没有问题，请你帮我预览一下，这个commit主要是为了fix KeyError: '((1, 4, 4), (1, 5, 5), ...)'


# round 2
好的，我觉得还是先添加一下诊断补丁吧，请帮我添加，还有一个点就是我觉得现在log的频率太低了，我希望能够log的频率高一些。
注意代码的可维护性和可读性。
[如有必要]改训练脚本和skill和readme，也请修改。

我还想问一个问题，在刚刚你的回复中，什么叫做只采样 scale0 的图？采样是指？

# round 3

注意看
var\local_output\stdout.txt @stdout.txt (4897-5019) 
我按照你的说法增加了diagnostics之后又跑了var，命令如下
var\README.md @README.md (185-201) 
我发现diagnostics中的重建图的二三列都是相当模糊，完全看不出一点样子（iter8710）和（iter0）都是一样的黑白噪声。但是第四列oracle rec和原图接近。

请帮我出谋划策一下我下一步应该怎么办。

【我感觉我可能需要把 scale0 起点改成 LR-conditioned query，而不是纯 SOS 扩展。例如从 low_f 通过轻量 projection/downsample 得到 [B, 16, D] 的 scale0 query，再加 pos_start。这比当前 sos.expand(B,16,D)+pos_start 更合理，只是个例子，我对深度学习理解肯定没你深刻，如果你有更好的办法，那就用你的，如果暂时不需要改，也请说明理由。】
我的理由如下，如今16个token都是一模一样（只有pos不一样），那么它们经过transform block 之后的输出几乎也是一摸一样的（因为qkv都一样），也就是说@SRVAR.py (768-796) 这里的输出几乎也是一摸一样的。
而target 4*4，明显是不一样的。我个人认为这明显是不合理的。

# round 4
好的，按照你的要求我跑了一下日志,如下：var\local_output\stdout.txt（6088-6123）
我发现即使跑到了iter2488，仍然根本没有在重建图中看到任何有效信息，依旧是无规律的黑白噪声，和iter0一模一样。

请问这是为什么？是还有关键点我没主意吗？（运行命令如下：var\README.md（191-210）

# round 5
好的，按照你刚刚的说法，我又跑了很久的实验，日志如下：var\local_output\stdout.txt（6088-6627）
我发现即使跑了很久，跑了快两个epoch了，仍然根本没有在重建图中看到任何有效信息，依旧是无规律的黑白噪声，和ep0-iter0一模一样。

还有一点，我觉得你刚刚说的optimizer step 后没有清梯度很有道理，请帮我改一下，关于其他原因也请你帮我仔细研究一下是为什么，并给我下一个运行的方案。我把调试结果给你

## round 5.1
好的，你说的没问题，不过我有几个疑问需要问一下：
1.刚跟你说 loss 已经不能证明采样链正常，反向 diffusion 在高噪声 timestep 仍会把 latent 放大到 decoder 饱和区。这句话是什么意思？能不能详细给我解释解释。
2.BS=64 AC=16这个参数是必要的吗？会不会拖慢训练进度？

# round 6
按照你刚刚的fix，我重新跑了很久的实验，日志如下：var\local_output\stdout.txt（？-8051）
我发现即使跑了很久，跑了快三个epoch了，仍然根本没有在scale0的diagnostics重建图中看到任何有效信息，依旧是无规律的黑白噪声，和ep0-iter0一模一样。（我非常确信） 参数是按照你刚刚的参数跑的。
重建图和metadata.json保存在var\local_output\srvar_scale0_lrq_equal_loss_fix_grad_bug

会不会是我的网络结构本身就有问题，不好训练？
请再次帮我分析各种原因，我明白这是一个困难的问题，不是轻易的可以找到答案，但是我希望你能够认真的思考，思考如何解决我的这个问题。如果问题在diffloss，diffloss暂时不好用，也可以暂时用其他更方便的方法代替，或者帮我改进diffloss

# round 7
报告！有重大成果，改成MSE之后，训练质量激增，很早就有效果了，效果还算可以！算是一个阶段性成功。
日志如下：var\local_output\stdout.txt（？-9434），PSNR达到了25、27左右。

现在我暂时的目标是：
先想办法提升一下效果。
具体怎么能够提升一下效果还需要你帮我想想办法，我只是有几个思路能够供你参考。


我的思路（仅供参考）：观察diagnostics图的时候我有几个发现：
1.AR_scale0_only | target_scale0_only 这两个很早就拟合了（差不多ep02000iter）的时候就基本上看不出什么区别了。但是它们的重建效果和LR_gt还是有很大的差别（如图）。这是正常的吗？我记得我训练vaex的时候，stage2的scale0是对齐stage1的LR latent的。所以我预想中的情况应该是，target_scale0_only应该和LR相近（至少）。你觉得我的思考合理吗？是否可以用这个切入点呢？
因为stage2 decoder在多尺度中是共享的，如果让适配stage1Decoder的LR latent在stage2的训练中硬着让stage2的scale0对齐stage1的LR latent反而不太好？（只是我的一些简单思考）。

你也可以不听我的话提出简介，先试试别的方法。总之我的目标是让效果变好

2.如果先不管vaex stage1、2的训练，那我们先不和HR_gt和LR_gt 去比.
我目前发现就算 AR_full和VAE_oracle在细节方面也差的比较多（或者可以使用diffloss？因为diffloss的提出就是为了丰富细节的，只不过之前没训练好）。

3.或者也可以先不硬着让stage2的scale0对齐stage1的LR训练出vae，再来试试也行。

想要一步登天总是困难的，需要慢慢前进！请你帮我想想我下一步应该干什么？要做哪些实验？

thank you！

注意你要站在我的角度来仔细思考问题，把自己当成主人公，来帮我想办法，而不只是完成任务。

# round 8

